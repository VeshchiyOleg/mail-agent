...
- Логи дублируются в `logs/mail-agent.log` (не только консоль) — проверено,
  что файл реально создаётся и совпадает по содержимому с консолью. Если
  запуск на защите не удастся и консоль будет недоступна/потеряна, этот
  файл — то, что нужно прислать для диагностики (плюс `data/audit.jsonl`,
  если процесс успел стартовать).
